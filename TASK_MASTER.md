...
### 2b. 书摘层（与制卡同步执行，A级场景专用）

书摘文件路径：`codex/excerpts/E-<册><章><场景>.md`（永不进 git，永不上传）

**五标准判定（全过才入选）**：

| 标准 | 核心问题 | 不过时的处理 |
|---|---|---|
| 一·不可转述性 | 用技法标签描述后，能照着写出等效句子吗？ | 能 → 只进卡片，不摘 |
| 二·惊讶度 | 读时"咦"了一下吗？（出乎预期的词/截断/焊接） | 没惊讶 → 你已会了，不摘 |
| 三·骨架可移植 | 剥人物情节后骨架仍成立吗？ | 不成立 → 标注`[仅供欣赏]`降权 |
| 四·填补空白 | 命中 `rules/user_gap_list.md` 中的弱项吗？ | 命中者优先级上浮 |
| 五·密度门槛 | 三行内叠了三种咬合手法吗？ | 单点金句不摘，记卡片即可 |

**每场景硬上限：5-8 条**，超出按 四>一>五 优先级砍。

**每条书摘格式**：
```
【第N条】L行号  [命中：a/b/c/d] [可移植/仅供欣赏·不可移植]
<原文段落完整截取>
→ 仿写时练什么：一句话
```

**user_gap_list.md 为活文档**：每轮试写揭示新弱项追加；连续两轮验证克服则移除。书摘筛选方向跟着成长曲线走。

**B级场景异常上报**：B级原则上不出书摘；若发现四关全过的句段，该场景标记"疑似重评"汇报 orchestrator。
